...
- **Tráfego ADS-B/MLAT**: acesso por link ao rastreador ao vivo do `adsb.lol`, já centrado no CIM. O site não incorpora nem reapresenta posições antigas como se fossem atuais. Não equivale à totalidade do tráfego e não é fonte oficial de separação ou autorização.
...
- `app.js` — clima, pista meteorológica e METAR
- `pista.js` — mapa e vento da view da pista
- `tests/audit.py` — validações estáticas e de consistência executadas no CI
